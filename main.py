...
def cetak(_temporary_tokens):
	strin = []
	for token_tag in _temporary_tokens:
		strin.append(unicode(token_tag[0].decode('utf-8')))

	return [(token) for (token) in _temporary_tokens]


f = open('data/data_korpus_ne2.txt')
...
print (chunker.parse(getPOSTag(word_tokenize('Kehadiran Jokowi disambut langsung oleh Menteri Perhubungan Ignasius Jonan dan Gubernur Kalimantan Utara Irianto Lambrie'
	))))
kalimat = "Kehadiran Jokowi disambut langsung oleh Menteri Perhubungan Ignasius Jonan dan Gubernur Kalimantan Utara Irianto Lambrie.".split(' ')
